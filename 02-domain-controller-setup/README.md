...
4. Renamed the server from the default autogenerated hostname `WIN-L06QGNMHTEP` to **DC-01** to follow standard domain controller naming conventions, then rebooted.
5. Logged in using the domain administrator account: **LAB\Administrator**.

> **Notes**
> - DNS is installed and configured automatically during domain promotion because Active Directory depends on DNS to locate domain services.
> - Renaming the domain controller early ensures consistent naming before additional domain resources and services are introduced.
